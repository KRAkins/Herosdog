...
async def search_google_by_image(image_url):
    try:
        ...
        if "items" in response:
            ...
            logger.error(f"Ошибка в ответе Google API: {response}")
            return None
...
async def search_prices_on_google(product_name):
    try:
        ...
        if "shopping_results" in response:
            return [(item["title"], item["link"], item.get("price", "N/A")) for item in response.get("shopping_results", [])]
        else:
            logger.error(f"Ошибка в ответе SerpAPI: {response}")
            return None
    except Exception as e:
        logger.error(f"Ошибка при поиске цен: {e}")
        return None
